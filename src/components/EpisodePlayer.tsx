...
export function EpisodePlayer({ item, onFinished }: { item: FeedItem; onFinished?: () => void }) {
  ...
  if (failed) {
    return (
      <p className="mt-3 text-[17px] leading-6 text-slate-700">
  ...
  return (
    <div className="mt-3 rounded-xl bg-slate-100 p-3">
      ...
      <div className="flex items-center gap-3">
        ...
        <button type="button" aria-label="Back 15 seconds" onClick={() => skip(-15)} className="shrink-0 text-[17px] font-semibold text-slate-700">−15s</button>
        <button type="button" aria-label="Forward 30 seconds" onClick={() => skip(30)} className="shrink-0 text-[17px] font-semibold text-slate-700">+30s</button>

        <span className="ml-auto shrink-0 tabular-nums text-[16px] text-slate-700">
