            ...
            {advisors.map((a) => {
              ...
              <Card key={a.id}>
                <div className="flex flex-wrap items-baseline gap-2">
                  <span className="text-[15px] font-semibold tracking-wide text-blue-600 uppercase dark:text-blue-400">
                    {a.seat}
                  </span>
                  <span className="font-semibold">{a.name}</span>
                  {fresh && (
                    <span className="rounded-md bg-blue-100 px-1.5 py-0.5 text-[15px] font-semibold text-blue-700 dark:bg-blue-950 dark:text-blue-300">
                      New post
                    </span>
                  )}
                </div>
                <p className="mt-1 text-[18px] text-slate-700 dark:text-slate-300">{a.lens}</p>
                <ul className="mt-2 space-y-1">
                  {a.questions.map((q) => (
                    <li key={q} className="text-[17px] leading-6 text-slate-700 dark:text-slate-400">
                      · {q}
                    </li>
                  ))}
                </ul>
                {fresh && (
                  ...
                  >
                    ...
                    <p className="mt-0.5 text-[18px] font-medium text-blue-700 dark:text-blue-300">{fresh.title}</p>
                  </a>
                )}
              </Card>
            )})}
              ...
              {sessions.map((s) => (
                ...
                >
                  ...
                  {s.actionItems.length > 0 && (
                    <p className="mt-1 text-[17px] text-blue-600 dark:text-blue-400">
  ...
  return (
    <>
      <Header
        title="Session"
        action={
          <button type="button" onClick={onClose} className="px-2 py-1 text-[19px] font-medium text-blue-600 dark:text-blue-400">
            Close
          </button>
        }
      />
          ...
          <section className="space-y-4">
            ...
            {chosen.map((a) => (
              <Card key={a.id} className="space-y-3">
                ...
                {a.questions.map((q, i) => (
                  <div key={q}>
                    <p className="mb-1.5 text-[18px] leading-6">{q}</p>
                    <Textarea
                      rows={2}
        ...
        {replies.length > 0 && (
          <section>
            ...
            <div className="space-y-2">
              {replies.map((a) => (
                <Card key={a.id}>
                  <p className="text-[16px] text-slate-600 dark:text-slate-500">{a.title}</p>
                  <p className="mt-1.5 text-[18px] leading-7 whitespace-pre-wrap">{a.reply}</p>
                </Card>
              ))}
            </div>
          </section>
          ...
          <section>
            ...
            <Card className="space-y-2">
              {existing.actionItems.map((it) => (
                <p key={it} className="text-[18px] leading-6">· {it}</p>
              ))}
              <p className="pt-1 text-[16px] text-slate-600 dark:text-slate-500">
